...
if __name__ == "__main__":
    ...
    # Erdos_Renyi version in NetworkX
    ti = time.time()
    Erdos_NetX = nx.erdos_renyi_graph(n=nodes, p=prob, seed=seed)
    tf = time.time()
    print(f"time for Erdos_NetX = {tf-ti} sec")
    ...
    print(f"nodes (numpy) = {net_numpy.get_numNodes()}")
    print(f"edges (numpy) = {net_numpy.get_numEdges()}")
